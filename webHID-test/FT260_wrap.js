...
class FT260Wrapper {
    ...
    static async ft260HidI2cWrite(device, i2cAddr, flag, data, bytesToWrite) {
        try {
            ...
            // Create fixed 63-byte HID report (excluding report ID)
    ...
    //         // const x = Math.floor((bytesToWrite - 1) / 4); // removed to test single report ID transfer
    //         const reportData = new Uint8Array(bytesToWrite + 4);
            
    //         // reportData[0] = 0xD0 + x;           // Report ID
    //         reportData[0] = 0xD0;           // Report ID, remove x to only use 0xD0
    ...
    static async ft260HidAddInputReportListener(device) {
        device.addEventListener("inputreport", event => {
            ...
            // Handle both FTDI and CircuitPython devices
            // NEW:
            const isValidDevice = FXCoreTargets.filters.some(filter => 
    ...
    static async requestDevice(filters) {
        try {
            ...
            // Opens a selector of HID devices that match the filter(s) 
            /*
